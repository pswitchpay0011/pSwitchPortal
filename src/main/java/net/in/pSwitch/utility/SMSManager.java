...
public class SMSManager {
    ...
    public String sendWelcomeMsg(UserInfo userInfo){
        Map<String, Object> request = new HashMap<>();
        request.put("apikey", "BxaypHaZNo6XVUYA");
        String otp = utilService.generateOTP();
        request.put("message", "Welcome "+userInfo.getFirstName()+"\n"
        		+ "Your agent ID is "+userInfo.getUserPSwitchId()+". Please use agent ID for login.\n"
        		+ "\n"
        		+ "PSWITCH");
        request.put("number", userInfo.getMobileNumber());
        request.put("senderid", "PSWTCH");
